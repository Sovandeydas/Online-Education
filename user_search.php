    ...
    <div class="content_box" style="height:500px;">
      ...
      <tr>
        ...
        <td><select name="keyword">
            ...
            <?php
				$keyword=mysql_query("SELECT DISTINCT `keyword` FROM `document`");
				if(mysql_num_rows($keyword))
				{
					while($drow=mysql_fetch_array($keyword))
					{
	  ...
	  if(isset($_POST['src']))
	  {
		  ...
		  $sql=mysql_query("SELECT d.*, u.fname,u.mname, u.lname ,t.tname, app.status, dw.nofd FROM document d INNER JOIN user u ON d.uid=u.uid INNER JOIN approve app ON d.did=app.did INNER JOIN type t ON d.tid=t.tid INNER JOIN download dw ON d.did=dw.did  WHERE app.status='Y' AND d.keyword='".$kw."'")or die(mysql_error());
		  if(mysql_num_rows($sql))
		  {
			  echo "<center>"."Total documents found ".mysql_num_rows($sql)."</center>";
			  ?><table cellpadding="7" cellspacing="0" border="1" style="margin:0 auto">
              	<th>Sl.No.</th><th>Uploaded By</th><th>Subject</th><th>Download</th><th>No. of Download</th>
                <?php
				$i=1;
