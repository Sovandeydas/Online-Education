    ...
    <div class="content_box" style="height:500px;">
     ...
     <center> <tr>
        ...
        <td><select name="tid">
        	...
        	<?php
		  $i=1;
		  while($drow=mysql_fetch_assoc($rs))
		  {
			  ?>
...
if(isset($_POST['ok']))
{
	...
	if($up==1)
	{
		...
		if($res==1)
		{
			$src=mysql_query("SELECT `did` FROM `document` WHERE `durl`='".$durl."'")or die(mysql_error()) ;
			if(mysql_num_rows($src)>0){
				$drow=mysql_fetch_assoc($src);
				$sql=mysql_query("INSERT INTO `approve`(`did`,`status`)VALUES(".$drow['did'].",'N')") or die(mysql_error());
				if($sql==1){
...
?>
      ...
      <center>
      <?php
	  $doc=mysql_query("SELECT * FROM `document` WHERE `uid`='".$_SESSION['u_info']['uid']."'")or die(mysql_error());
	  if(mysql_num_rows($doc))
	  {
		  ?>
          <div style="height:20px; padding-top:20px;">
		  <?php
		  echo "Total documents uploaded ".mysql_num_rows($doc);
		  ?></div>
		  ...
		  <?php
		  $i=1;
		  while($drow=mysql_fetch_array($doc))
		  {
			  ?>
              <tr>
              <td><?php echo $i ?></td>
	  ...
	  {
		  $ddid=$_GET['did'];
		  $del=mysql_query("DELETE FROM `document` WHERE `did`='$ddid'") or die(mysql_error());
		  if($del==1)
		  {
